...
class MultilayerPerceptron:
    ...
    def thetas_init(layers):
        """
        初始化权重参数，对于每一层,随机生曾一个小的值作为权重参数矩阵，其中偏置向的个数为输出点
        :param layers:
        :return:
        """
    ...
    def gradient_descent(data, labels, unrolled_theta, layers, max_iterations, alpha):
        """
        使用梯度下降算法更新权重参数。在训练过程中，
        每次迭代都计算当前权重参数下的损失函数，
        并根据梯度下降算法更新权重参数，使得损失函数逐渐减小。
        :param data:
        :param labels:
        :param unrolled_theta:
        :param layers:
    ...
    def back_propagation(data, labels, thetas, layers):
        ...
        首先进行前向传播算法得到每一层的激活值。
        然后计算输出层和真实标签之间的差异，
        用反向传播算法逐层计算每层的误差，并根据误差计算权重参数梯度。最终返回各层权重参数梯度。
        :param data:
        :param labels:
        :param thetas:
        :param layers:
    ...
    def feedforward_propagation(data, thetas, layers):
        """
        前向传播算法，计算输出结果。对于每一个样本，
        首先将其作为输入层的激活值，然后通过每层的权重参数矩阵与上一层的激活值相乘并加上偏置项，进行线性变换。
        然后，将线性变换的结果通过sigmoid激活函数进行非线性变换，得到下一层的激活值。最终返回输出层的激活值，即预测结果
        :param data:
        :param thetas:
        :param layers:
        :return:
